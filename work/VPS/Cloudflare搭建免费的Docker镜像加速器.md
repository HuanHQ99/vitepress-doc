本期内容主要介绍使用 cloudflare 的 workers 功能实现免费的 Docker加速代理服务。

注意事项：若是其他平台购买的域名或免费域名，将其托管到cloudflare上的，在后续绑定域名的时候可能无法访问。

项目地址https://github.com/iCloudBot/DockerRescue
1、登录Cloudflare平台
地址：https://www.cloudflare.com/zh-cn/

点击 “Workers和Pages” →  “创建Worker”
![本地图片](/public/img/work/Cloudflare搭建免费的Docker镜像加速器/1.png)
填写自定义的 Worker名称，点击 “部署”。
![本地图片](/public/img/work/Cloudflare搭建免费的Docker镜像加速器/2.png)
2、部署Docker加速器
进入到部署成功页面后，再点击 “编辑代码”。
![本地图片](/public/img/work/Cloudflare搭建免费的Docker镜像加速器/3.png)
将下面代码复制粘贴到初始化代码中，进行替换。内容无需做任何的修改。
```yaml
// Docker镜像仓库主机地址
...
````
替换后，点击右上角的“部署” → “保存并部署”，提示部署成功。
![本地图片](/public/img/work/Cloudflare搭建免费的Docker镜像加速器/4.png)
有代理的小伙伴，可以打开截图中，第二个箭头标的的url查看部署后的效果。

没有代理的，可不用点击，待绑定好域名后，再去打开。
![本地图片](/public/img/work/Cloudflare搭建免费的Docker镜像加速器/5.png)
3、域名绑定
返回到 Workers 和 Pages 页面，点击 设置 → 触发器 → 添加自定义域

添加在 cloudflare 上的域名，填写 二级、三级、四级...都可以。
![本地图片](/public/img/work/Cloudflare搭建免费的Docker镜像加速器/6.png)
点击添加后，填写你的域名，点击右下方添加域
![本地图片](/public/img/work/Cloudflare搭建免费的Docker镜像加速器/7.png)
4、配置加速器
登录到服务器之后，复制下面命令到终端执行（将域名更改为你的域名）。
...
````
```yaml
sudo systemctl daemon-reload #重载 systemd 管理守护进程配置文件
...
````
```yaml
docker pull docker.nastool.de/library/alpine:latest
